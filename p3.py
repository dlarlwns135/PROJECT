...
class Idle:
    ...
    def enter(p3, e):
        ...
        # elif left_down(e):
        #     p3.left = True
        #p3.wait_time = get_time() # pico2d import 필요
        pass
    ...
    def draw(p3):
        if p3.dir == -1:
            p3.idle.clip_composite_draw(int(p3.frame) * 32, 0, 32, 64, 0, 'h', p3.x, p3.y + 20, 100, 200)
        elif p3.dir == 1:
            p3.idle.clip_composite_draw(int(p3.frame) * 32, 0, 32, 64, 0, '', p3.x, p3.y + 20, 100 , 200)
...
class Run:
    ...
    def enter(p3, e):
        ...
        if p3.right and not p3.left:
            ...
            p3.dir = -1
        elif not p3.right and p3.left:
            p3.dir = -1
        elif not p3.left and p3.right:
            p3.dir = 1
    @staticmethod
    def exit(p3, e):
        p3.y += 15
    ...
    def draw(p3):
        if p3.dir == -1:
            p3.run.clip_composite_draw(int(p3.frame) * 64, 0, 64, 32, 0, 'h', p3.x, p3.y, 200, 100)
        elif p3.dir == 1:
            p3.run.clip_composite_draw(int(p3.frame) * 64, 0, 64, 32, 0, '', p3.x, p3.y, 200, 100)
...
class Jump:
    ...
    def do(p3):
        ...
        if p3.y <= ground_y:
            ...
            # if p3.jump_move:
            #     pass
            #     # p3.state_machine.handle_event(('JUMP_END_RUN', None))
            #     # print("JUMP_END_RUN")
            # else:
...
class Teleport:
    ...
    @staticmethod
    def do(p3):
        p3.frame = p3.frame + 4 * 5 * game_framework.frame_time
        if p3.frame >= 3:
...
class Skill_motion:
    ...
    @staticmethod
    def draw(p3):
        # p3.attack4.clip_composite_draw(p3.frame * 72, 0, 72, 66, 0, 'h', p3.x - 62, p3.y - 3, 225, 206)
        if p3.skill_num == 2:
            if p3.dir == -1:
            ...
            if p3.jump_state:
                if p3.dir == -1:
                    p3.skill1_jump.clip_composite_draw(int(p3.frame) * 40, 0, 40, 64, 0, 'h', p3.x, p3.y, 125, 200)
                elif p3.dir == 1:
                    p3.skill1_jump.clip_composite_draw(int(p3.frame) * 40, 0, 40, 64, 0, '', p3.x, p3.y, 125, 200)
            else:
                if p3.dir == -1:
                    p3.skill1_stand.clip_composite_draw(int(p3.frame) * 40, 0, 40, 64, 0, 'h', p3.x, p3.y, 125, 200)
                elif p3.dir == 1:
                    p3.skill1_stand.clip_composite_draw(int(p3.frame) * 40, 0, 40, 64, 0, '', p3.x, p3.y, 125, 200)
        pass
...
class P3:
    ...
    def __init__(self):
        ...
        self.frame = 0
        self.dir = 1
        self.idle = load_image('itachi_idle.png')
